...
- [x] v1.4アップデート — 完了 (2026-06-12, コミット 7661f82〜)
  ...
  - ゲーム名は「星の階」に変更済み
- [ ] v1.6実験(不採用・mainはv1.5) — ブランチ v1.6-battle-inversion に全8コミット保存
  - 戦闘反転(攻vs攻の敗北=攻撃側非破壊/攻vs守の敗北=攻撃側破壊「守備の反撃」)
    +カード7種改訂+BuffEntry.defAmount/effectiveDef。同値処理は3パターン試行
    (相打ち→双方破壊なし→攻撃側押し切り/相討ち)
  - 不採用理由: テストプレイで改悪と判断。計測でも先攻勝率39.7〜44.5%(後攻優位)、
    normalとhardの拮抗、膠着増(デッキ切れ7%)が確認された。詳細はブランチ内の
    ルールブック_v1.6.md 付録C参照
- [ ] v1.5.2実験(不採用・mainはv1.5) — ブランチ v1.5.2-summon-unification に保存
  - 召喚統合・「以下」ルール。ぴったりの手応え喪失+先攻勝率悪化(44.1%)で不採用
- [x] v1.5アップデート — 完了 (コミット 2f42ed2, 9556f91, b7e0905) **← mainの現行版**
