...
- [x] v1.4アップデート — 完了 (2026-06-12, コミット 7661f82〜)
  ...
  - ゲーム名は「星の階」に変更済み
- [x] v1.6アップデート — 完了 (コミット b890b9b, a52fb22, ddf6996)
  - 戦闘反転(守備の反撃): 攻vs攻の敗北=攻撃側非破壊、攻vs守の敗北=攻撃側破壊。
    BuffEntry.defAmount+effectiveDef新設(草薙剣・毒酒が攻守両方に)
  - カード7種改訂(軍配+500/人魚の肉2500/背水の陣=レベル以下守備/
    天罰=お互い全破壊/アイギス=場が空の時のみ)
  - CPUは警戒対象を攻守で交換。強行弁をデッキ残り8枚に前倒し(膠着対策)
  - 追補: 同値処理の変遷 = 相打ち(bfa4c59で双方破壊なし)→ 攻vs攻=攻撃側が
    押し切り(防御側のみ破壊)/攻vs守=相討ち(両者破壊)。いずれもダメージなし
  - 既知の課題①: 先攻勝率44.1%(後攻+12pt、構造的)。ライフ補償は効かないことを
    実測済み(+1000/+1500とも誤差範囲)→ 対策するなら別レバーが必要(付録C参照)
  - 既知の課題②: normalとhardが拮抗(hard 49.7%/2600戦)。同値タダ取りが
    normalの単純スタイルを強化したため。罠で主力を守る補正は効果なし(膠着増)で見送り
- [ ] v1.5.2実験(不採用) — ブランチ v1.5.2-summon-unification に保存
  - 召喚統合・「以下」ルール。ぴったりの手応え喪失+先攻勝率悪化で不採用決定
